...
Trained on the [Emotions dataset](https://huggingface.co/datasets/dair-ai/emotion) (16,000 labeled sentences across 6 classes), evaluated on a 20% held-out test split.
...
| Metric | Baseline | Balanced (shipped model) |
|---|---|---|
| Overall accuracy | 86.3% | **88.5%** |
| Love recall | 61.5% | **91.2%** |
| Surprise recall | 46.9% | **85.8%** |
...
2. The text is lowercased and cleaned (URLs and punctuation stripped)
3. The cleaned text is transformed into TF-IDF features using the saved vectorizer
4. The trained Logistic Regression model predicts the emotion class and its probability distribution
5. Results are displayed as a labeled prediction plus a visual spectrum of all six emotion confidences
...
1. Clone the repository:
   ```bash
   git clone https://github.com/jaswantgopala/Emotion-Detector.git
   cd Emotion-Detector
   ```
...
4. Open the URL shown in the terminal (typically `http://localhost:8501`) in your browser.
